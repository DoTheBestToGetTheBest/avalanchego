...
func (*network) CrossChainAppRequestFailed(context.Context, ids.ID, uint32, error) error {
...
func (*network) AppRequestFailed(context.Context, ids.NodeID, uint32, error) error {
	// This VM currently only supports gossiping of txs, so there are no
	// requests.
	return nil
}
